 ...
 * License: Apache-2.0
 *
 */var h=m;function j(T,u=!1){let r=D(T);if(typeof r=="string"||!r[1]||r[1]==="")return"";let s=r,i=(function(){return s[0]==""?T:s[0]})(),n="TZID="+i,a=[`BEGIN:VTIMEZONE\r
TZID:`+i+`\r
X-LIC-LOCATION:`+i+`\r
LAST-MODIFIED:`+s[1].replace(/[^\w\-:,;=+/<>]/g,"").replace(/<br>/g,`\r
`)+"END:VTIMEZONE",n];return u?JSON.stringify(a):a}function I(T,u,r){let s=D(T);if(typeof s=="string"||s[1]==null||s[1]=="")return"";let i=s;if(!u.match(/^\d{4}-\d{2}-\d{2}$/))return console.error("offset calculation failed: date misspelled [-> YYYY-MM-DD]"),"";if(!r.match(/^\d{2}:\d{2}$/))return console.error("offset calculation failed: time misspelled [-> hh:mm]"),"";if(!i[1].match(/BEGIN:DAYLIGHT/i)){let Y=i[1].match(/TZOFFSETTO:([+|-]\d{4})/i);return Y?Y[1]:""}let n=u+"T"+r+":00",a=new Date(n),e=a.getFullYear(),t=a.getMonth()+1,c=a.getDate(),A=a.getHours(),b=i[1].replace(/[^\w\-:,;=+/<>]/g,"").split("<br>"),o={1:{},2:{}},l=0;for(let Y=0;Y<b.length;Y++){let f=b[Y];if(f.startsWith("TZOFFSETTO")&&(l++,o[`${l}`]&&(o[`${l}`].offset=f.split(":")[1])),f.startsWith("DTSTART")&&o[`${l}`]&&(o[`${l}`].hour=parseInt(f.substr(17,2))),f.startsWith("RRULE")){let z=f.split(";"),k=z[1].split("=")[1],L=parseInt(k);o[`${l}`]&&(o[`${l}`].month=L,o[`${l}`].day=z[2].split("=")[1])}}o[1].month!==void 0&&o[2].month!==void 0&&o[1].month>o[2].month&&([o[1],o[2]]=[o[2],o[1]]);let d=o[1],E=o[2];if(d.month===void 0||E.month===void 0||d.offset===void 0||E.offset===void 0)return"";if(t!=d.month&&t!=E.month)return t<d.month||t>E.month?E.offset:d.offset;let B=(function(){return Object.keys(o).find(Y=>o[`${Y}`].month==t)})();if(!B)return E.offset;let M=o[`${B}`],y=["SU","MO","TU","WE","TH","FR","SA"],_=new Date(e,t,0).getDate(),S=new Date(e,t-1,1).getDay(),R={SU:{},MO:{},TU:{},WE:{},TH:{},FR:{},SA:{}};for(let Y=1;Y<=_;Y++){let f=y[S],z=Object.keys(R[`${f}`]).length+1;R[`${f}`][`${z}`]=Y,S++,S==7&&(S=0)}let p=(function(){if(!M.day)return 0;if(M.day[0]=="-"){let Y=M.day.substr(2,2),f=Object.keys(R[`${Y}`]).length+1-parseInt(M.day[1]);return R[`${Y}`][`${f}`]}else{let Y=M.day.substr(1,2);return R[`${Y}`][M.day[0]]}})();if(c>p||c==p&&A>=(M.hour||0))return M.offset||"";let N=(function(){return B=="1"?"2":"1"})();return o[`${N}`].offset||""}function $(T=!1){let u=[],s=(n=>{let a={};for(let[e,t]of Object.entries(n))if(typeof t=="object"&&!Array.isArray(t)){a[`${e}`]={};let c=t;for(let[A,b]of Object.entries(c))if(typeof b=="object"&&!Array.isArray(b)){a[`${e}`][`${A}`]={};let o=b;for(let[l,d]of Object.entries(o)){let E=d,B=E[0].replace(new RegExp(`^${g(e)}/${g(A)}/`),`${h.toplevel.indexOf(e)}/${A}/`);a[`${e}`][`${A}`][`${l}`]=[B,E[1]]}}else{let o=b,l=o[0].replace(new RegExp(`^${g(e)}/`),`${h.toplevel.indexOf(e)}/`);a[`${e}`][`${A}`]=[l,o[1]]}}else{let c=t,A=c[0].replace(new RegExp(`^${g(e)}/`),`${h.toplevel.indexOf(e)}/`);a[`${e}`]=[A,c[1]]}return a})(h.db),i=(n,a="")=>{for(let[e,t]of Object.entries(n))typeof t=="object"&&!Array.isArray(t)?i(t,a+e+"/"):u.push(a+e)};return i(s),T?JSON.stringify(u):u}export{j as tzlib_get_ical_block,I as tzlib_get_offset,$ as tzlib_get_timezones};
