 ...
 * License: Apache-2.0
 *
 */var h=m;function $(i,e=!1){let t=p(i);if(typeof t=="string"||!t[1]||t[1]==="")return"";let T=t,s=(function(){return T[0]==""?i:T[0]})(),n="TZID="+s,a=[`BEGIN:VTIMEZONE\r
TZID:`+s+`\r
X-LIC-LOCATION:`+s+`\r
LAST-MODIFIED:`+T[1].replace(/[^\w\-:,;=+/<>]/g,"").replace(/<br>/g,`\r
`)+"END:VTIMEZONE",n];return e?JSON.stringify(a):a}function w(i,e,t){let T=p(i);if(typeof T=="string"||T[1]==null||T[1]=="")return"";let s=T;if(!e.match(/^\d{4}-\d{2}-\d{2}$/))return console.error("offset calculation failed: date misspelled [-> YYYY-MM-DD]"),"";if(!t.match(/^\d{2}:\d{2}$/))return console.error("offset calculation failed: time misspelled [-> hh:mm]"),"";if(!s[1].match(/BEGIN:DAYLIGHT/i)){let u=s[1].match(/TZOFFSETTO:([+|-]\d{4})/i);return u?u[1]:""}let n=e+"T"+t+":00",a=new Date(n),r=a.getFullYear(),Y=a.getMonth()+1,c=a.getDate(),A=a.getHours(),b=s[1].replace(/[^\w\-:,;=+/<>]/g,"").split("<br>"),o={1:{},2:{}},l=0;for(let u=0;u<b.length;u++){let f=b[u];if(f.startsWith("TZOFFSETTO")&&(l++,o[`${l}`]&&(o[`${l}`].offset=f.split(":")[1])),f.startsWith("DTSTART")&&o[`${l}`]&&(o[`${l}`].hour=parseInt(f.substr(17,2))),f.startsWith("RRULE")){let z=f.split(";"),L=z[1].split("=")[1],H=parseInt(L);o[`${l}`]&&(o[`${l}`].month=H,o[`${l}`].day=z[2].split("=")[1])}}o[1].month!==void 0&&o[2].month!==void 0&&o[1].month>o[2].month&&([o[1],o[2]]=[o[2],o[1]]);let d=o[1],E=o[2];if(d.month===void 0||E.month===void 0||d.offset===void 0||E.offset===void 0)return"";if(Y!=d.month&&Y!=E.month)return Y<d.month||Y>E.month?E.offset:d.offset;let B=(function(){return Object.keys(o).find(u=>o[`${u}`].month==Y)})();if(!B)return E.offset;let M=o[`${B}`],_=["SU","MO","TU","WE","TH","FR","SA"],N=new Date(r,Y,0).getDate(),S=new Date(r,Y-1,1).getDay(),R={SU:{},MO:{},TU:{},WE:{},TH:{},FR:{},SA:{}};for(let u=1;u<=N;u++){let f=_[S],z=Object.keys(R[`${f}`]).length+1;R[`${f}`][`${z}`]=u,S++,S==7&&(S=0)}let Z=(function(){if(!M.day)return 0;if(M.day[0]=="-"){let u=M.day.substr(2,2),f=Object.keys(R[`${u}`]).length+1-parseInt(M.day[1]);return R[`${u}`][`${f}`]}else{let u=M.day.substr(1,2);return R[`${u}`][M.day[0]]}})();if(c>Z||c==Z&&A>=(M.hour||0))return M.offset||"";let k=(function(){return B=="1"?"2":"1"})();return o[`${k}`].offset||""}function K(i=!1){let e=[],T=(n=>{let a={};for(let[r,Y]of Object.entries(n))if(typeof Y=="object"&&!Array.isArray(Y)){a[`${r}`]={};let c=Y;for(let[A,b]of Object.entries(c))if(typeof b=="object"&&!Array.isArray(b)){a[`${r}`][`${A}`]={};let o=b;for(let[l,d]of Object.entries(o)){let E=d,B=E[0].replace(new RegExp(`^${g(r)}/${g(A)}/`),`${h.toplevel.indexOf(r)}/${A}/`);a[`${r}`][`${A}`][`${l}`]=[B,E[1]]}}else{let o=b,l=o[0].replace(new RegExp(`^${g(r)}/`),`${h.toplevel.indexOf(r)}/`);a[`${r}`][`${A}`]=[l,o[1]]}}else{let c=Y,A=c[0].replace(new RegExp(`^${g(r)}/`),`${h.toplevel.indexOf(r)}/`);a[`${r}`]=[A,c[1]]}return a})(h.db),s=(n,a="")=>{for(let[r,Y]of Object.entries(n))typeof Y=="object"&&!Array.isArray(Y)?s(Y,a+r+"/"):e.push(a+r)};return s(T),i?JSON.stringify(e):e}return G(x);})();
